...
const FollowupDealers = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      // Calculate stats
      const overdue = dealersList.filter(
        (d) => d.followup_status === "overdue",
      ).length;
      const today = dealersList.filter(
        (d) => d.followup_status === "today",
      ).length;
      const upcoming = dealersList.filter(
        (d) => d.followup_status === "upcoming",
      ).length;
      ...
      toast.error(
        error.response?.data?.detail || "Failed to fetch followup dealers",
      );
    } finally {
      setLoading(false);
    }
  ...
  const toggleOrderedItem = (item) => {
    ...
    const isSelected = followupData.ordered_items.some(
      (i) => i.name === itemName,
    );
  ...
  const handleUpdateFollowup = async () => {
    ...
    if (
      followupData.outcome === "Follow-up Required" &&
      !followupData.next_visit_date
    ) {
    ...
    try {
      ...
      if (error?.code === 1) {
        toast.error(
          "Location permission denied. Please allow location access to record visits.",
        );
        return;
      }
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <p className="text-sm font-semibold text-gray-800">
                    {stats.overdue} Overdue Follow-up
                    {stats.overdue !== 1 ? "s" : ""}
                  </p>
                  <p className="text-xs text-gray-600 mt-1">
                    These dealers are past their scheduled follow-up date.
                    Please visit them as soon as possible.
                  </p>
                </div>
              </div>
            </CardContent>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(val) =>
                  setFollowupData({ ...followupData, outcome: val })
                }
              >
                <SelectTrigger>
