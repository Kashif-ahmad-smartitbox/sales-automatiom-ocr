...
const UserVisitSummary = () => {
  ...
  const selectSession = (userId, sessionId, e) => {
    ...
    setSelectedSession((prev) => ({
      ...prev,
      [userId]: prev[userId] === sessionId ? null : sessionId,
    }));
    setPage((prev) => ({ ...prev, [userId]: 1 }));
  };
  ...
  const getTerritoryName = (id) => {
    ...
    return (
      tByName?.name || (id.length < 30 && !id.includes("-") ? id : "Unknown")
    );
  };
      ...
      <div className="space-y-2" data-testid="user-visit-summary">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-0.5">
            Dealers shown when market starts vs visited. Expand each user to see
            full dealer table (Dealer page layout).
          </p>
        </div>
          ...
          <Card className="border-0 shadow-sm">
            <CardContent className="py-12 text-center">
              ...
              <p className="text-sm text-gray-500">
                No sales executives or visit data yet
              </p>
            </CardContent>
          </Card>
        ) : (
                  ...
                  {expandedUser === user.user_id && (
                    <div className="border-t border-gray-100">
                      {false && user.market_sessions?.length > 0 && (
                        <div className="px-4 py-3 bg-gray-50/50 border-b border-gray-100">
                          <p className="text-[10px] font-semibold text-gray-600 mb-2 flex items-center gap-2">
                            <Funnel size={12} /> Market Sessions (click to
                            filter table)
                          </p>
                          <div className="flex flex-wrap gap-2">
                            <button
                              type="button"
                              onClick={(e) =>
                                selectSession(user.user_id, null, e)
                              }
                            ...
                            {(showAllSessions[user.user_id]
                              ? user.market_sessions
                              : user.market_sessions.slice(0, SESSIONS_INITIAL)
                            ).map((s) => (
                              <button
                                key={s.id}
                                type="button"
                                onClick={(e) =>
                                  selectSession(user.user_id, s.id, e)
                                }
                              ...
                              >
                                ...
                                <span className="text-primary-600 font-medium">
                                  Shown: {s.dealers_shown}
                                </span>
                                <span className="text-gray-400 mx-2">•</span>
                                <span className="text-emerald-600 font-medium">
                                  Visited: {s.dealers_visited}
                                </span>
                              </button>
                            ))}
                            {user.market_sessions.length > SESSIONS_INITIAL && (
                              <button
                                type="button"
                                onClick={(e) =>
                                  toggleShowAllSessions(user.user_id, e)
                                }
                              ...
                              >
                                {showAllSessions[user.user_id] ? (
                                  ...
                                  <>
                                    {" "}
                                    <CaretDoubleDown size={12} /> Show all (
                                    {user.market_sessions.length})
                                  </>
                                )}
                              </button>
                            )}
                      ...
                      {(() => {
                        ...
                        const filteredDealers = !user.dealers?.length
                          ? []
                          : sid
                            ? user.dealers.filter((d) =>
                                (d.session_ids || []).includes(sid),
                              )
                            : user.dealers;
                        const currentPage = page[user.user_id] || 1;
                        const totalPages = Math.max(
                          1,
                          Math.ceil(filteredDealers.length / PAGE_SIZE),
                        );
                          ...
                          <div className="overflow-auto rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-sm w-full max-h-[30rem]">
                            <Table className="table-auto border-separate border-spacing-0">
                              <TableHeader className="text-nowrap sticky top-0 text-xs z-10">
                                ...
                                <TableHead className="p-2 text-gray-500 font-semibold text-center bg-gray-200 border-b border-gray-200">
                                  Items
                                </TableHead>
                              </TableHeader>
                              <TableBody className="divide-y divide-gray-100 dark:divide-gray-800">
                                {filteredDealers.length === 0 ? (
                                  ...
                                  paginatedDealers.map((dealer, idx) => {
                                    ...
                                    const foundBy = getTruncatedText(
                                      dealer.found_by,
                                      15,
                                    );
                              ...
                              <div className="flex items-center justify-between px-4 py-3 border-t border-gray-100 bg-gray-50/50">
                                ...
                                <div className="flex items-center gap-1">
                                  <Button
                                    ...
                                    onClick={() =>
                                      setUserPage(user.user_id, currentPage - 1)
                                    }
                                  >
                                    <CaretLeft size={14} />
                                  </Button>
