...
const AssignedPotentials = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const [potentialsRes, itemsRes] = await Promise.all([
        axios.get(`${API}/visit/my-assigned-potentials`, {
      ...
      toast.error(
        error.response?.data?.detail || "Failed to fetch assigned dealers",
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleSaveVisit = async () => {
    ...
    if (
      visitData.outcome === "Follow-up Required" &&
              ...
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className="text-sm font-semibold text-gray-800">
                    Ready to Visit These Dealers?
                  </p>
                  <p className="text-xs text-gray-600 mt-1">
                    These assigned dealers will automatically appear in your
                    Field View when you start a market visit.
                  </p>
                </div>
                <Button
                ...
                <TableBody className="divide-y divide-gray-100 dark:divide-gray-800">
                  {loading ? (
                    ...
                    <TableRow>
                      <TableCell
                        colSpan="7"
                        className="px-2 py-8 text-center text-[11px] text-gray-500 dark:text-gray-400"
                      >
                        {searchTerm
                          ? "No matches found."
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(val) =>
                  setVisitData({ ...visitData, outcome: val })
                }
              >
                <SelectTrigger>
                  ...
                  {companyProducts.length > 0 ? (
                    <div className="border border-gray-200 rounded-lg p-2 max-h-[300px] overflow-y-auto bg-gray-50/50 space-y-2">
                      {companyProducts.map((item) => {
                        const itemName = item.item_name;
                        const isSelected = visitData.ordered_items.some(
                          (i) => i.name === itemName,
                        );
                        return (
                          <div
