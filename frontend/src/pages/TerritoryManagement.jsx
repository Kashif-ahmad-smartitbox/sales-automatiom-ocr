...
const TerritoryManagement = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        await axios.put(`${API}/territories/${editingId}`, payload, {
          headers: getAuthHeader(),
        });
  ...
  const handleDelete = async (id) => {
    if (
      !window.confirm("Are you sure? This will also affect child territories.")
    )
      return;
    try {
      ...
      <div className="space-y-4" data-testid="territory-management">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">
              Define your territory hierarchy: State → City → Area → Beat
            </p>
          </div>
          ...
          >
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>
                  {editingId ? "Edit Territory" : "Add New Territory"}
                </DialogTitle>
              </DialogHeader>
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="flex justify-end gap-2 pt-4">
                  ...
                  <Button
                    ...
                    data-testid="territory-submit-btn"
                  >
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {["State", "City", "Area", "Beat"].map((type, idx) => (
            <Card
              key={type}
              className={`border-0 shadow-md hover:shadow-lg transition-all duration-300 text-white ${
                idx === 0
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {["State", "City", "Area", "Beat"].map((type) => (
              <Card
                ...
                data-testid={`territory-${type.toLowerCase()}-card`}
              >
                <CardContent className="p-3">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <Badge
                      className={typeColors[type] + " text-[10px] px-1.5 py-0"}
                    >
                      {groupedTerritories[type].length}
                    </Badge>
                  </div>
                  ...
                  {groupedTerritories[type].length === 0 ? (
                    ...
                    <div className="space-y-1">
                      {groupedTerritories[type].map((territory) => (
                        <div
                          key={territory.id}
                          className="flex items-center justify-between px-2 py-1 bg-gray-50 rounded border border-gray-100 hover:border-primary-200 transition-colors"
                        >
                          <div>
                            ...
                            {territory.parent_id && (
                              <p className="text-[10px] text-gray-500">
                                Parent: {getParentName(territory.parent_id)}
                              </p>
                            )}
                          </div>
