...
function setupDevServer(config) {
  config.setupMiddlewares = (middlewares, devServer) => {
    ...
    const isAllowedOrigin = (origin) => {
      ...
      // Allow all smartItbox.sh subdomains
      if (origin.match(/^https:\/\/([a-zA-Z0-9-]+\.)*smartItbox\.sh$/)) {
        return true;
      }

      // Allow all smartItboxagent.com subdomains
      if (origin.match(/^https:\/\/([a-zA-Z0-9-]+\.)*smartItboxagent\.com$/)) {
        return true;
      }
      ...
      try {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          try {
            ...
            execSync(
              `git -c user.name="visual-edit" -c user.email="support@smartItbox.sh" add "${targetFile}"`
            );
            execSync(
              `git -c user.name="visual-edit" -c user.email="support@smartItbox.sh" commit -m "visual_edit_${timestamp}"`
            );
          } catch (gitError) {
